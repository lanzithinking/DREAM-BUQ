...
for n in range(n_dif):
    ...
    # test difference
    dif_n = dll_xact - img2fun(dll_emul,elliptic.pde.V).vector()
    dif[n] = dif_n.norm('l2')/dll_xact.norm('l2')
...
#     print('Relative difference between finite difference and extracted results: {}'.format(reldif))
    ...
    # plot
    if n in sel4print:
        print('Difference between the calculated and emulated gradients: min ({}), med ({}), max ({})'.format(dif_n.min(),np.median(dif_n.get_local()),dif_n.max()))
        plt.subplot(121)
...
act_str=','.join([val.name if callable(val) else val for val in activations.values()])
dif_sumry=[dif.min(),np.median(dif),dif.max()]
dif_str=np.array2string(np.array(dif_sumry),precision=2,separator=',').replace('[','').replace(']','')
# dif_str=np.array2string(np.array(dif_sumry),precision=2,separator=',',formatter={'float': '{: 0.2e}'.format}).replace('[','').replace(']','')
sumry_header=('Time','num_filters','activations','latent_dim','droprate','dif (min,med,max)')
sumry_np=np.array([ctime,flt_str,act_str,latent_dim,droprate,dif_str])
