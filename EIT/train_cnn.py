...
latent_dim=256 # best for non-whiten 256
...
cnn=CNN(x_train.shape[1:], y_train.shape[1], num_filters=num_filters, latent_dim=latent_dim, droprate=droprate,
        activations=activations, optimizer=optimizer, kernel_initializers=kernel_initializers)
...
for n in range(n_dif):
    ...
    # calculate gradient
    t_start=timeit.default_timer()
    ll_xact,dll_xact = eit.get_geom(eit.img2vec(u),[0,1],force_posperm=True)[:2]
...
# emulate gradient
dll_emul = eit.img2vec(cnn.gradient(eit.vec2img(eit._sim2pts(u))[None,:,:,None], logLik))
