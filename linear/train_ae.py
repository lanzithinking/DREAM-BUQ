...
# X=loaded['X']
import pickle
with open('./analysis/lin_infHMC_dim3_2020-12-22-23-59-22.pckl','rb') as f:
...
# import pandas as pd
test=pd.DataFrame(x_test,columns=['$u_{}$'.format(i) for i in np.arange(1,d+1)])
reconstructed=pd.DataFrame(ae.model.predict(test),columns=['$u_{}$'.format(i) for i in np.arange(1,d+1)])
