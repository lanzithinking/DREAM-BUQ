...
# define DNN
depth=3
# node_sizes=[lin.input_dim,8,16,32,64,lin.output_dim]
...
# loglik = lambda y: lin.logpdf(y)[0]
loglik = lambda y: -0.5*tf.math.reduce_sum((y-lin.y[None,:])**2/lin.nz_var[None,:],axis=1)
# custom_loss = lambda y_true, y_pred: [tf.square(loglik(y_true)-loglik(y_pred)), (y_true-y_pred)/lin.nz_var[None,:]]
# dnn=DNN(x_train.shape[1], y_train.shape[1], depth=depth, droprate=droprate,
